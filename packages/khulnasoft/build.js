...
const runCommands = {
    win32: {
        ...
        link: () => shell.exec('mklink /D "%SPLUNK_HOME%\\etc\\apps\\khulnasoft" "%cd%\\..\\..\\output\\khulnasoft"'),
    },
    nix: {
        build: () => shell.exec('export NODE_ENV=production && ./node_modules/.bin/webpack --mode=production'),
        unlink: () => shell.exec('unlink $SPLUNK_HOME/etc/apps/khulnasoft || true'),
        link: () => shell.exec('ln -s $PWD/../../output/khulnasoft $SPLUNK_HOME/etc/apps/khulnasoft'),
    },
};
